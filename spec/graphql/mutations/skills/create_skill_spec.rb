  ...
  describe '#resolve' do
    ...
    describe 'happy path' do
      ...
      describe 'sad path' do
        ...
        it 'if no skill not included, returns error' do
          @query = <<~GQL
                        mutation {
                          skill: createSkill(
                            input: {
          ...
          data = json['errors']
          expect(data[0]['message']).to eq("Argument 'astrogation' on InputObject 'CreateSkillInput' has an invalid value (\"\"). Expected type 'Int!'.")
          expect(Skill.count).to eq(0)
        end
      end
    end
